...
public class TrainView implements TrainViewCallBack {
    ...
    private void onTrain(List<Train> allSchedule) {
        ...
        do {
            System.out.println("----------------------------------------------------------------");
            System.out.println("1. Train route and timings");
            ...
            choice = scanner.nextInt();
            switch(choice) {
                case 1: showStations(allSchedule);break;
                case 2: init(); break;
    ...
    private void showStations(List<Train> allSchedule) {
        ...
        for (Train train : allSchedule) { 
            if(train.getTrainNumber() == trainNo) {
                trainController.getStations(trainNo);
                return;
            }
        }
    }
    ...
    public void displayStations(List<Stations> stations) {
        ...
        for (Stations station : stations) {
            if(!station.getArrivalTime().startsWith("-")) System.out.print(station.getArrivalTime() + "\t\t");
            else System.out.print("\t\t");
            System.out.print(station.getStationName() + "\t\t");
            if(!station.getDepartureTime().startsWith("-")) System.out.println(station.getDepartureTime());
        }
    }
}
