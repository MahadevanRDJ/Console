...
public class AccountView implements AccountViewCallBack {
    ...
    public void newAccount() {
        ...
        do { 
            ...
            choice = scanner.nextInt();
            switch(choice) {
                case 1: if(!isPinSet) setPin(); break;
                case 2: if(isPinSet) init();; break;
    ...
    private void setPin() {
        ...
        if(pin == rPin) accountController.setPin(accountNumber, pin);
        isPinSet = true;
    }
    ...
    private boolean checkAccount(int accountNumber) {
        System.out.println("----------------------------------------------------------------");
        if(accountController.checkAccount(accountNumber)) return true;
        else return false;
    }
    @Override
    public void onAccount(Account account) {
        int choice;
        if(account.getPin() == 0) newAccount(); 
        do {
            System.out.println("----------------------------------------------------------------");
            System.out.println("1. Account Details");
    ...
    private void depositMoney(Account account) {
        ...
        int amount = getInt();
        accountController.depositMoney(accountNumber, amount);
    }
    ...
    private void transferMoney(Account account) {
        ...
        if(checkAccount(targetAccountNumber)) {
            System.out.println("Amount :");
            int amount = getInt();
            accountController.transferMoney(sourceAccountNumber,targetAccountNumber,amount);        
        }
    }

    private void transactionHistory(Account account) {
        accountController.transactionHistory(account.getAccountNumber());
    }
    private int getInt() {
    ...
    public void showTransactionHistory(List<Transactions> transactions) {
        ...
        try{
            ...
            System.out.println(transaction.getTransactionTime() + "\t\t" + transaction.getTransactionID() + "     \t"+ transaction.getTransactionType());
            }
        } catch(Exception e) {
