...
public class CarParkingRepository {
    ...
    public void setInitialSlot() {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            List<Integer> currentDaySlots = new ArrayList<Integer>();
            for (int k = 0; k < 24; k++) {
                currentDaySlots.add(0);
            }
            allDaySlots.get(i).setSlots(currentDaySlots);
            perDaySlot.add(i, currentDaySlots);
        }
    ...
    public boolean removeSlot(String slotDate, int slotTime) {
        ...
        if (userSlot.containsKey(slotDate.toUpperCase())) {
            for (int i = 0; i < 24; i++) {
                if (i == slotTime && allDaySlots.get(date).getSlots().get(i) == 1) {
